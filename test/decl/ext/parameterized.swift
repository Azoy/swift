...
_ = o == p // expected-error {{binary operator '==' cannot be applied to two 'Pair<Wrapped<Int>>' operands}}
...
_ = r.compacted() // expected-error {{generic parameter 'T' could not be inferred}}
                  // expected-error@-1 {{referencing instance method 'compacted()' on 'Collection' requires the types 'Int' and 'T?' be equivalent}}
